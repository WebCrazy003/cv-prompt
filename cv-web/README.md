...
A local-only React and Fastify application that runs repository CV skills through one authenticated Codex App Server process. Each of the five application tabs owns an independent Codex thread and filesystem snapshot, so five job applications can run concurrently without sharing inputs or outputs.
