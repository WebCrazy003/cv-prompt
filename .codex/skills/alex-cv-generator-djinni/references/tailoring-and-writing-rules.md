...
- Do not add a JD technology absent from the Djinni profile merely for ATS coverage. If a broader supplied fact truthfully subsumes the term, use only the supported level of specificity.
...
Organize skills into ATS-readable categories represented by `categoryName` and `skillItems`. Each category must render naturally as `Category: item, item, item`. Include only job-relevant skills supported by the Djinni profile or conservative permitted completion.
...
- Use direct, conversational language and no more than 10 words per answer.
