...
- Read candidate facts from `Alex L/candidate-profile-sebastian-no-LK.md`.
...
- At the start of every invocation, read `Alex L/candidate-profile-sebastian-no-LK.md` and `Alex L/job-application.md` from disk, even when their contents appeared earlier in the conversation or a previous invocation.
...
8. Run `python3 .codex/skills/sebastian-cv-generator-no-lk/scripts/validate_employers.py "Alex L/candidate-profile-sebastian-no-LK.md" "Alex L/cv-output.json"` and treat any missing, extra, or repeated employer as a validation failure.
