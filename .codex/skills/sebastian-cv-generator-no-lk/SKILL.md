...
description: Generate Sebastian Sygula's complete, strongly tailored ATS-friendly CV JSON from isolated runtime inputs and the no-LK profile. Use when tailoring Sebastian's CV without LinkedIn-sourced facts, answering application questions, or auditing CV quality.
---
...
Read candidate facts from `base-profile/sebastian/candidate-profile-sebastian-no-LK.md`. Treat identity, education, employer names, and employment dates in that file as fixed facts. Use the supplied job description and professional context to complete missing career details. Never modify candidate or runtime input files.
...
8. Run `python3 .codex/skills/sebastian-cv-generator-no-lk/scripts/validate_employers.py "base-profile/sebastian/candidate-profile-sebastian-no-LK.md" "<cvOutputFile>"`, substituting the quoted placeholder with its invocation value; treat any missing, extra, or repeated employer as a failure.
